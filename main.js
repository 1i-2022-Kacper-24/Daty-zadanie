...
const selectCalendar = document.getElementById('selectDate');
...
if (calendar.style.display == 'none'){
    howMany = Date(select.value);
}
else if (calendar.style.display == 'block'){
    howMany = Date(selectCalendar.value);
}
 

wydarzeniaSzkolne.forEach(function (event){
    const option = document.createElement('option');
    option.value = event.data;
    option.textContent = event.wydarzenie;
...
btnToday.addEventListener('click', function(){
    ...
    if (calendar.style.display == 'block'){
        howMany = new Date(selectCalendar.value);
        let left = new Date();
        left = Math.floor((left - howMany) / (1000*60*60*24) * (-1));
        if (left < 0){
            left = left * (-1);
            amount.innerText = "Minęło " + left + " dni";
        }
        else {
            amount.innerText = "Pozostało " + left + " dni";
    }
    }
    else {
        howMany = new Date(select.value);
        let left = new Date();
        left = Math.floor((left - howMany) / (1000*60*60*24) * (-1));
        if (left < 0){
            left = left * (-1);
